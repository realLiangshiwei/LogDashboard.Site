...
namespace LogDashboard.Site
{
    ...
    public class SiteWebModule : AbpModule
    {
        public override void PreConfigureServices(ServiceConfigurationContext context)
        {
            context.Services.PreConfigure<AbpMvcDataAnnotationsLocalizationOptions>(options =>
            {
                options.AddAssemblyResource(
                    typeof(SiteResource),
                    typeof(SiteDomainModule).Assembly,
                    typeof(SiteApplicationModule).Assembly,
                    typeof(SiteWebModule).Assembly
                );
            });
        }
        ...
        private static void ConfigureAutoMapper(IServiceCollection services)
        {
            ...
            {
                options.AddProfile<SiteWebAutoMapperProfile>();
            });
        }
        ...
        private static void ConfigureVirtualFileSystem(IServiceCollection services, IHostingEnvironment hostingEnvironment)
        {
            if (hostingEnvironment.IsDevelopment())
            {
                ...
                {
                    options.FileSets.ReplaceEmbeddedByPyhsical<SiteDomainModule>(Path.Combine(hostingEnvironment.ContentRootPath, string.Format("..{0}LogDashboard.Site.Domain", Path.DirectorySeparatorChar)));
                });
            }
        }
        ...
        private static void ConfigureNavigationServices(IServiceCollection services)
        {
            ...
            {
                options.MenuContributors.Add(new SiteMenuContributor());
            });
        }
        ...
        private static void ConfigureAutoApiControllers(IServiceCollection services)
        {
            ...
            {
                options.ConventionalControllers.Create(typeof(SiteApplicationModule).Assembly);
            });
        }
        ...
        private static void ConfigureSwaggerServices(IServiceCollection services)
        {
            services.AddSwaggerGen(
                options =>
                {
                    options.SwaggerDoc("v1", new Info { Title = "Site API", Version = "v1" });
        ...
        public override void OnApplicationInitialization(ApplicationInitializationContext context)
        {
            ...
            {
                options.SwaggerEndpoint("/swagger/v1/swagger.json", "Site API");
            });
